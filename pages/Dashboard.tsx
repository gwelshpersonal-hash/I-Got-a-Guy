...
export const Dashboard = () => {
  ...
  const submitRequest = () => {
      ...
      if (isUrgent) {
          // Send Text Alerts to opted-in providers WHO HAVE THE SKILL
          const eligibleProviders = users.filter(u => 
              u.role === Role.PROVIDER && 
              u.isActive && 
              u.urgentAlertsEnabled &&
              u.skills?.includes(selectedCategory) // Targeted Broadcast
          );
  ...
  const renderGigCard = (gig: Shift) => {
      ...
      const breakdown = calculateJobSplit(gig.price || 0, gig.category, isVerifiedInsured);
                ...
                {myActiveGigs.length === 0 ? (
                    ...
                    <div className="space-y-4">
                        {myActiveGigs.map(gig => {
                            ...
                            return (
                            <div key={gig.id} className={`p-6 rounded-2xl shadow-soft border transition-all hover:shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white border-transparent`}>
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-2"><span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider ${gig.status === 'OPEN_REQUEST' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>{gig.status.replace('_', ' ')}</span><span className="text-xs text-slate-500 font-medium">Posted {gig.createdAt ? formatDistanceToNow(gig.createdAt) : 'recently'} ago</span>{gig.type === 'URGENT' && <span className="text-[10px] font-bold bg-red-600 text-white px-2 py-0.5 rounded-full animate-pulse">URGENT</span>}</div>
